...
#include <string.h>
#include <stdio.h>
#include "/home/jiang/armvisintsoft/include/visint.h"
#include <unit.h>
/*---------------------------------------------------------
 * PRIVATE METHODS
 *--------------------------------------------------------- */
...
clish_view_t *clish_command__set_name(struct clish_shell_s   *this, clish_command_t *cmd ,char *str)
{

    struct clish_view_s *view= cmd->view;
    ...
    if( strcmp(cmd->name ,"session-module") == 0 )
    {
        str = str+15 ;// delete name=

        saveSlotid = atoi(str);


        if( status[saveSlotid]== 0)
        {
            printf("this bload isn't exist\n");
            view = clish_shell_find_view(this,"enable-view");
            return view;
        }
        visModuleClass = status[saveSlotid];
        if( visModuleClass == olp )
        {
            saveSlotType = BLOAD_OLP;
			if(getUnitType(0,saveSlotid)== OTU_FOUR)
            {
                sprintf(buf,"olp-view");
            }
            else if(getUnitType(0,saveSlotid)== OTU_EIGHT||getUnitType(0,saveSlotid)==OTU_CTYPE||getUnitType(0,saveSlotid)==OLPD_EIGHT)
            {

                sprintf(buf,"olpo-view");
            }
			else if(getUnitType(0,saveSlotid)== SEESION_OBP)
            {
            	sprintf(buf,"obp-view");
            }

        }
        else if( visModuleClass == otu )
        {
            saveSlotType = BLOAD_OPS;
            if(getUnitType(0,saveSlotid)== OTU_FOUR)
            {
                sprintf(buf,"otu-view");
            }
            else if(getUnitType(0,saveSlotid)== OTU_EIGHT)
            {

                sprintf(buf,"otueight-view");
            }
			else if(getUnitType(0,saveSlotid)== OTU_LIU_FOUR)
            {
		...
		else if( visModuleClass == BLOAD_DVOA )
        {
            saveSlotType = BLOAD_DVOA;

            sprintf(buf,"dvoa-view");
  
        }
		else if( visModuleClass == oeo )
        {
            
            if(getUnitType(0,saveSlotid)== OEO_FOUR)
            {
                sprintf(buf,"oeo-view");
            }
            else if(getUnitType(0,saveSlotid)== OEO_EIGHT)
            {

                sprintf(buf,"oeoeight-view");
            }
		    else if(getUnitType(0,saveSlotid)== OEO_BEIGHT)
            {

                sprintf(buf,"oeobeight-view");
            }
        }
        else if( visModuleClass == BLOAD_OSS )
        {
            saveSlotType = BLOAD_OSS;
            sprintf(buf,"oss-view");
        }
        else if( visModuleClass == BLOAD_EDFA )
        {
            saveSlotType = BLOAD_EDFA;
            sprintf(buf,"edfa-view");
        }
		else if( visModuleClass == BLOAD_ETU )
        {
            saveSlotType = BLOAD_EDFA;
            sprintf(buf,"etu-view");
        }
        else if( visModuleClass == BLOAD_PASSIVE )
        {
            if(getUnitType(0,saveSlotid)==BLOAD_OSU)
            {
                sprintf(buf,"osu-view");
            }
            else if (getUnitType(0,saveSlotid)==BLOAD_ODM)
            {
                sprintf(buf,"odm-view");
            }
			else if (getUnitType(0,saveSlotid)==BLOAD_AWG)
            {
                sprintf(buf,"awg-view");
            }
        }
        else if( visModuleClass == BLOAD_CONVERTOR )
        {
            sprintf(buf,"convert-view");
        }
        else if( visModuleClass == BLOAD_OBS )
        {
            sprintf(buf,"ops-view");
        }
        else if( visModuleClass == BLOAD_MCU )
        {
            sprintf(buf,"convert-view");
        }
        else
        ...
        return clish_shell_find_view(this,buf);
    }
